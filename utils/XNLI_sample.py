...
def generate_XNLI_examples_Train(filepath):
    """This function returns the examples in the raw (text) form."""
    vocab = WordLevel.read_file("/Users/barid/Documents/workspace/alpha/UNIVERSAL/vocabulary/EnDeHi_6k_wiki/EnDeHi_6K_vocab.json")
    bpe_tok = WordLevel(vocab, unk_token="[UNK]")
    vocab_tokenizer = Tokenizer(bpe_tok)
    bpe_tok = fastBPE.fastBPE("/Users/barid/Documents/workspace/alpha/UNIVERSAL/vocabulary/EnDeHi_6k_wiki/EnDeHi_6k_code")
    def _encode(string):
        ...
        with open('nli_test_label' + '_' + LANG, "w") as label:
            ...
            for row in rows_per_pair_id:
                ...
                if row['gold_label'] == "neutral":
                    ...
                    gold_label = 2
                nli.write("3@@2 " + " ".join([str(p) for p in preHypo]))
                nli.write("\n")
                label.write(str(gold_label))
                label.write("\n")
    nli.close()
