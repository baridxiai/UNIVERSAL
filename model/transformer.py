...
class Transformer(NaiveSeq2Seq):
    ...
    def encoding(self, inputs, attention_bias=0, training=False, enc_position=None, vis=False):
        ...
        with tf.name_scope("encoding"):
            ...
            for index in range(self.param["num_encoder_layers"]):
                src = self.encoder[index](src, attention_bias=attention_bias, training=training, index=index,)
                if vis:
    ...
    def decoding(
        ...
        with tf.name_scope("decoding"):
            ...
            for index in range(self.param["num_decoder_layers"]):
