# -*- coding: utf-8 -*-
# code warrior: Barid
# A folk of https://github.com/tensorflow/addons/pull/2525/files
...
class GradientAccumulator(tf.keras.optimizers.Optimizer):
            ...
            for grad, var in grads_and_vars:
                ...
                if isinstance(grad, tf.IndexedSlices):
                    ...
                    def _get_grad():
                        ...
                        indices = tf.squeeze(
                            tf.where(
                                tf.reduce_sum(new_grad, axis=list(range(len(new_grad.shape))[1:]))
                                != 0
                            ),
                            axis=-1,
                        )
                        ...
                        handle.assign(
                            tf.zeros_like(handle), use_locking=self._use_locking, read_value=False,
                        )
                    ...
                    values, indices, dense_shape = tf.cond(
                        ...
                        _get_grad,
                        lambda: (tf.zeros_like(grad.values), grad.indices, grad.dense_shape,),
                    )
                    new_grad = tf.IndexedSlices(values, indices, dense_shape)
                    new_grads_and_vars.append((new_grad, var))
                else:
                    handle.assign_add(grad, use_locking=self._use_locking, read_value=False)
                    ...
                    new_grad = tf.cond(
                        self.step % self._accum_steps == 0, _get_grad, lambda: tf.zeros_like(grad),
                    )
                    new_grads_and_vars.append((new_grad, var))
    ...
    def _resource_apply_sparse_duplicate_indices(self, grad, handle, indices, apply_state=None):
        if "apply_state" in self._optimizer._sparse_apply_args:
        ...
        with tf.control_dependencies([self.step.assign_add(1, read_value=False)]):
            ...
            with tf.control_dependencies([train_op]):
                return self.iterations.assign_sub(
                    tf.cast(self.step % self._accum_steps != 0, tf.int64), read_value=False,
                )
